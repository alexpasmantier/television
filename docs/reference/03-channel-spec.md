...
| `sort` | string | No | Ranking profile for results. `default` uses generic fuzzy ranking; `prefer_prefix` can add a bounded bonus to matches that start closer to the beginning of an entry, and frecency still applies. `history` preserves source order for score ties, disables that bonus, and still allows frecency. `source` preserves source order exactly and ignores both `prefer_prefix` and frecency. Supported values: `default`, `history`, `source` (default: `default`) |
| `prefer_prefix` | boolean | No | Give matches near the start of an entry an extra ranking bonus when `sort = "default"` (default: true). This is not a hard prefix-only ordering rule. Ignored by `history` and `source`. |

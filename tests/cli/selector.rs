...
/// Test edge case: more selections than placeholders (should use first N entries)
#[test]
fn test_preview_command_excess_selections() {
    let mut tester = PtyTester::new();
    let temp_config = TempConfig::init();

    // Create a channel with preview command expecting 2 placeholders
    temp_config
        .write_channel(
            "preview-excess-test",
            r#"
                [metadata]
                name = "preview-excess-test"
                description = "Test channel for excess selections"

                [ui]
                results_max_selections = 4

                [source]
                command = ["echo -e 'file1.txt\nfile2.txt\nfile3.txt\nfile4.txt'"]

                [preview.command]
                template = "echo 'Only: {} and {}'"  # 2 placeholders
                mode = "one_to_one"
                separator = ""
                shell_escaping = false
            "#,
    ...
    tester.assert_tui_frame_contains("preview-excess-test");

    // Select 3 items (more than 2 placeholders)
    tester.send(&ctrl('i')); // Select file1.txt and move down
    tester.send(&ctrl('i')); // Select file2.txt and move down
    tester.send(&ctrl('i')); // Select file3.txt and move down

    // Should use first 2 entries only
    tester.assert_tui_frame_contains("Only: file1.txt and file2.txt");

    tester.send(&ctrl('c'));
    PtyTester::assert_exit_ok(&mut child, DEFAULT_DELAY * 2);
}

/// Test edge case: fewer selections than placeholders (should handle gracefully)
#[test]
fn test_preview_command_insufficient_selections() {
    let mut tester = PtyTester::new();
    let temp_config = TempConfig::init();

    // Create a channel with preview command expecting 3 placeholders
    temp_config
        .write_channel(
            "preview-insufficient-test",
            r#"
                [metadata]
                name = "preview-insufficient-test"
                description = "Test channel for insufficient selections"

                [source]
                command = ["echo -e 'alpha.txt\nbeta.txt\ngamma.txt'"]

                [preview.command]
                template = "echo 'A: {} B: {} C: {}'"  # 3 placeholders
                mode = "one_to_one"
                separator = ""
                shell_escaping = false
            "#,
    ...
    // Select only 2 items (fewer than 3 placeholders)
    tester.send(&ctrl('i')); // Select alpha.txt and move down
    tester.send(&ctrl('i')); // Select beta.txt and move down

    // Should show an empty preview
    tester.assert_tui_frame_contains("│>                                                   3 / 3 ││                                                          │");

    tester.send(&ctrl('i')); // Select gamma.txt and move down

    // Should show selected items
    tester.assert_tui_frame_contains("A: alpha.txt B: beta.txt C: gamma.txt");
